...
pub fn parse_fips_school_id(input: &str) -> FIPSParseResult<FIPSCode> {
  ...
  if rest.starts_with("x") {
    ...
    let fips_code = FIPSCode::new(state, county, 0, SettingCategory::PrivateSchool, school_id, 0);
    Ok((rest, fips_code))
  } else {
    // Public school 
    ...
    let fips_code = FIPSCode::new(state, county, tract, SettingCategory::PublicSchool, school_id, 0);
    Ok((rest, fips_code))
  }
}
...
pub fn parse_fips_workplace_id(input: &str) -> FIPSParseResult<FIPSCode> {
  ...
  let fips_code = FIPSCode::new(state, county, tract, SettingCategory::Workplace, workplace_id, 0);
  Ok((rest, fips_code))
}
...
pub fn parse_home_id(input: &str) -> FIPSParseResult<IdCode> {
  parse_decimal_digits_to_bits(4, 14, input).map(|(rest, value)| {
    (rest, value as IdCode)
  })
}
...
pub fn parse_private_school_id(input: &str) -> FIPSParseResult<IdCode> {
  ...
  parse_decimal_digits_to_bits(4, 11, input).map(|(rest, value)| {
    (rest, value as IdCode)
  })
}
...
pub fn parse_workplace_id(input: &str) -> FIPSParseResult<IdCode> {
  parse_decimal_digits_to_bits(5, 14, input).map(|(rest, value)| {
    (rest, value as IdCode)
  })
}
...
pub fn parse_integer(input: &str) -> FIPSParseResult<u64> {
  ...
  if digit_end == 0 {
    return Err(
      (
        input,
        FIPSParserError::InvalidLength { expected: 1, found: 0 }
      )
    );
  }
...
mod tests {
  ...
  fn test_parse_integer() {
    ...
    // Long number
    assert_eq!(parse_integer("18446744073709551615end"), Ok(("end", 18446744073709551615))); // u64 max
  ...
  fn test_combined_scenarios() {
    ...
    assert_eq!(parse_workplace_id("19876@#$%"), Err(("19876@#$%", FIPSParserError::ValueExceedsCapacity{value:19876, capacity: 16383})) );
