...
A table of how FIPS GeoIDs are structured is provided in the module-level documentation for [`crate::parser`] (slightly
modified from
[the source table in the standard](https://www.census.gov/programs-surveys/geography/guidance/geo-identifiers.html)).
The rows in the table up to and including Block (that is, all but the last
...
use crate::{states::USState, CountyCode, DataCode, IdCode, SettingCategoryCode, StateCode, TractCode, CATEGORY_OFFSET, COUNTY_OFFSET, FOURTEEN_BIT_MASK, FOUR_BIT_MASK, ID_OFFSET, SIX_BIT_MASK, STATE_OFFSET, TEN_BIT_MASK, TRACT_OFFSET, TWENTY_BIT_MASK};
use std::{
    fmt::{
        Debug,
        Display,
...
mod tests {
    ...
    fn test_set_id() {
        ...
        let fips_code = FIPSCode::with_category(USState::AK, 0, 0, SettingCategory::CensusTract.into());
